<template>
  <v-card color="transparent" flat>
    ...
    <v-card-text class="pa-0 pb-2">
      <div>
        <span>{{ $vuetify.lang.t(`$vuetify.message.school.score`, $formatNum(message.score, true)) }}</span>
        <span v-if="message.perfectScore">&nbsp;{{ $vuetify.lang.t(`$vuetify.message.school.perfectScore`) }}</span>
      </div>
      <div v-if="message.points > 0">+{{ $formatInt(message.points) }} {{ $vuetify.lang.t(`$vuetify.school.totalPoints`) }}</div>
