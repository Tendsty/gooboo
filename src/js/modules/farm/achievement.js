...
export default {
    ...
    maxOvergrow: {value: () => store.state.stat.farm_maxOvergrow.total, display: 'percent', milestones: lvl => getSequence(2, lvl + 1) * 0.5 + 1, relic: {4: 'lightningRod'}},
    bestPrestige: {value: () => store.state.stat.farm_bestPrestige.total, cap: 18, milestones: lvl => lvl * 2 + 6},
    ...
    care: {value: () => store.state.stat.farm_care.total, cap: 10, milestones: lvl => Math.round(Math.pow(lvl + 2, 2) * Math.pow(1.4, lvl) * 10)},
}
